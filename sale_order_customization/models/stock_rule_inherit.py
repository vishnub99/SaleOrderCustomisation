    ...
    def _run_pull(self, procurements):
        ...
        for company_id, moves_values in moves_values_by_company.items():
            ...
            for i_product in res:
                ...
                if origin:
                    sale_id = self.env['sale.order'].sudo().search([('name', '=', origin)], limit=1)
                    if sale_id.auto_workflow:
            ...
            if origin:
                sale_id = self.env['sale.order'].sudo().search([('name', '=', origin)], limit=1)
                if sale_id.auto_workflow:
                    inv_id = (self.env['sale.advance.payment.inv'].with_context
                        (active_ids=[sale_id.id]).sudo().create({
                            'advance_payment_method': 'delivered',
                        }))
                    inv_id.create_invoices()
                    inv_id.sale_order_ids.invoice_ids.action_post()
                    self.env['account.payment.register'].with_context(active_model='account.move',
                                                                      active_ids=inv_id.sale_order_ids.invoice_ids.ids).create(
                        {
                            'payment_date': inv_id.sale_order_ids.invoice_ids.date,
                        })._create_payments()
